...
## Find the un-traversed users in the approvers database
findDoc = {"traversed":False}

## This result contains all of the unique approvers in the databaase
results = approvers.find(findDoc)

## Now for each approver we are creating a profile for that github user with his name, profile creation time, number of repos, the language the user has most repos in, and for each repo with their languages
approversInserted = 0
for result in results:
	...
	## Updating all the approver documents with traversed:true so that in future they do not have to be traversed again
	findDoc = {"_id":ObjectId(str(id))}
	updateDoc = {"$set":{"traversed":True}}
	res = db.approvers.update(findDoc,updateDoc)
	...
	if not resultExists:
		...
		if not isDeletedUser:
			...
			try:
				...
				approversInserted += 1
				print approversInserted, " ",current_mongo_insert_id
			except:
